...
try:
    ...
    print("Error while importing JSON")
...
ROUTERFILE = 'equip_r.txt'
SWITCHFILE = 'equip_s.txt'
...
def main():
    ...
    with open(ROUTERFILE) as RouterF:
        RouterContents = json.load(RouterF)
    with open(SWITCHFILE) as SwitchF:
        SwitchContents = json.load(SwitchF)
    
    #dictionaries
    ##---->>>> read the routers and addresses into the router dictionary

    routers = RouterContents
    
    ##---->>>> read the switches and addresses into the switches dictionary

    switches = SwitchContents
    ...
    with open(UPDATEDFILE, 'w') as UpdatedContents:
        json.dump(updated, UpdatedContents)
    ...
    with open(ERRORFILE, 'w') as ErrorContents:
        json.dump(invalidIPAddresses, ErrorContents)            
